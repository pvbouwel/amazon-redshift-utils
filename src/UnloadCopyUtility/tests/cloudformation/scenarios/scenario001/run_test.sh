...
cat >${HOME}/scenario001.json <<EOF
{
  ...
  "s3Staging": {
    ...
    "path": "s3://${CopyUnloadBucket}/scenario001/",
...
#Extract DDL
psql -h ${SourceClusterEndpointAddress} -p ${SourceClusterEndpointPort} -U ${SourceClusterMasterUsername} ${SourceClusterDBName} -c "select ddl from admin.v_generate_tbl_ddl where schemaname='ssb' and tablename='dwdate';" | awk '/CREATE TABLE/{flag=1}/ ;$/{flag=0}flag' | sed 's/ssb/public/' >${HOME}/scenario001.ddl.sql 2>>${STDERROR}
...
psql -h ${TargetClusterEndpointAddress} -p ${TargetClusterEndpointPort} -U ${TargetClusterMasterUsername} ${TargetClusterDBName} -f ${HOME}/scenario001.ddl.sql | grep "CREATE TABLE"  >>${STDOUTPUT} 2>>${STDERROR}
r=$? && stop_step $r


start_step "Run Unload Copy Utility"
source ${VIRTUAL_ENV_PY36_DIR}/bin/activate >>${STDOUTPUT} 2>>${STDERROR}
cd ${HOME}/amazon-redshift-utils/src/UnloadCopyUtility && python3 redshift_unload_copy.py --log-level debug ${HOME}/scenario001.json eu-west-1 >>${STDOUTPUT} 2>>${STDERROR}
