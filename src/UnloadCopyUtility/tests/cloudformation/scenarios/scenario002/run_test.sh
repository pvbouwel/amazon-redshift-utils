...
start_scenario "Perform Unload Copy with password encrypted using KMS, expect target location to be correct on Python2"
start_step "Create configuration JSON to copy ssb.dwdate of source cluster to public.dwdate on target cluster"
cat >${HOME}/scenario002.json <<EOF
{
  "unloadSource": {
  ...
  "s3Staging": {
    ...
    "path": "s3://${CopyUnloadBucket}/scenario002/",
...
cat ${HOME}/scenario002.json >>${STDOUTPUT} 2>>${STDERROR}
r=$? && stop_step $r

start_step "Generate DDL for table public.dwdate on target cluster"
#Extract DDL
psql -h ${SourceClusterEndpointAddress} -p ${SourceClusterEndpointPort} -U ${SourceClusterMasterUsername} ${SourceClusterDBName} -c "select ddl from admin.v_generate_tbl_ddl where schemaname='ssb' and tablename='dwdate';" | awk '/CREATE TABLE/{flag=1}/ ;$/{flag=0}flag' | sed 's/ssb/public/' >${HOME}/scenario002.ddl.sql 2>>${STDERROR}
increment_step_result $?
cat ${HOME}/scenario002.ddl.sql >>${STDOUTPUT} 2>>${STDERROR}
increment_step_result $?
...
psql -h ${TargetClusterEndpointAddress} -p ${TargetClusterEndpointPort} -U ${TargetClusterMasterUsername} ${TargetClusterDBName} -f ${HOME}/scenario002.ddl.sql | grep "CREATE TABLE" >>${STDOUTPUT} 2>>${STDERROR}
r=$? && stop_step $r


start_step "Run Unload Copy Utility"
source ${VIRTUAL_ENV_PY27_DIR}/bin/activate >>${STDOUTPUT} 2>>${STDERROR}
cd ${HOME}/amazon-redshift-utils/src/UnloadCopyUtility && python2 redshift_unload_copy.py --log-level debug ${HOME}/scenario002.json eu-west-1 >>${STDOUTPUT} 2>>${STDERROR}
